...
@interface NSAudioHandlerKit () <AVAudioPlayerDelegate, AVAudioRecorderDelegate>
...
@implementation NSAudioHandlerKit
...
- (void)record {
    ...
    [[AVAudioSession sharedInstance] requestRecordPermission:^(BOOL granted) {
        if (granted) {
            ...
            _isRecording = NO;
            [NSObject showWithTitle:@"提示" message:@"没有权限" cancelButtonTitle:@"知道了" otherButtonTitles:nil andAction:nil andParentView:nil];
            //            kTipAlert(@"没有权限");
        }
    }];
}
...
- (void)stopRecord:(BOOL)successfully {
    ...
    if (_isRecording) {
        ...
        if (duration < _minRecordDuration) {
            ...
            NSString *recordFile = [NSAudioFileProcesser encodeWaveToAmr:_tmpFile];
