...
#import "NSMenuItem.h"
#import "NSMenuContainerView.h"
#import "NSMenuOperation.h"

typedef NS_ENUM(NSInteger, NSMenuViewArrowDirection) {
    NSMenuViewArrowDirectionNone,
    NSMenuViewArrowDirectionUp,
    NSMenuViewArrowDirectionDown,
};

@interface NSMenuView()
@property (strong, readwrite, nonatomic) NSMenuOperation *menu;

@property (nonatomic, assign) NSMenuViewArrowDirection arrowDirection;//箭头方向
...
- (void) setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect
{
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) { //箭头point下方有足够距离
        ...
        _arrowDirection = NSMenuViewArrowDirectionUp;
        ...
        _arrowDirection = NSMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = NSMenuViewArrowDirectionNone;
             ...
             menuItems:(NSArray *)menuItems
{
    ...
    NSMenuContainerView *overlay = [NSMenuContainerView menuWithFrame:view.bounds coverStyle:MenuCoverColor_Gray];
//    [[NSMenuContainerView alloc] initWithFrame:view.bounds];
...
- (void)menuItemClick:(UIButton *)sender
{
    ...
    NSMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *)makeContentView{
    ...
    //获得最大item的宽度和高度
    for (NSMenuItem *menuItem in _menuItems) {
    ...
    for (NSMenuItem *menuItem in _menuItems) {
        ...
        //自定义样式
        if (_menu.menuBackgroundStyle == NSMenuBackgroundStyleDark) {
            button.backgroundColor = BlackForMenu;
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        }else if(_menu.menuBackgroundStyle == NSMenuBackgroundStyleLight){
        ...
        if (itemNum < _menuItems.count - 1) {
            ...
            if (_menu.menuSegmenteLineStyle == NSMenuSegmenteLineStylefollowContent) {
            ...
            }else if (_menu.menuSegmenteLineStyle == NSMenuSegmenteLineStyleFill) {
...
- (CGPoint) arrowPoint
{
    ...
    if (_arrowDirection == NSMenuViewArrowDirectionUp) {
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition,CGRectGetMinY(self.frame) };
    } else if (_arrowDirection == NSMenuViewArrowDirectionDown) {
