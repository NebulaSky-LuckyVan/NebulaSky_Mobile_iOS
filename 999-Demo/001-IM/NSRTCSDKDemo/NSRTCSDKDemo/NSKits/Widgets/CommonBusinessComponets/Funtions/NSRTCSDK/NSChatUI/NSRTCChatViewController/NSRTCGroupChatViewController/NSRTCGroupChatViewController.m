//
//  NSRTCGroupChatViewController.m
//  NSRTCSDKDemo
//
...
@interface NSRTCGroupChatViewController () <UITableViewDelegate, UITableViewDataSource, NSRTCMessageInputViewDelegate, UIScrollViewDelegate, NSRTCMessageCellDelegate ,TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
...
@property (strong, nonatomic) NSRTCGroupChatViewModel *viewModel;
...
@implementation NSRTCGroupChatViewController
+ (instancetype)groupChatWithUsers:(NSArray<NSString *> *)toUsers{
    NSRTCGroupChatViewController *chatVc = [[NSRTCGroupChatViewController alloc]init];
    chatVc.users = [toUsers copy];
    return chatVc;
}
- (instancetype)init{
    self = [super init];
...
- (void)setupViewModel{
    self.viewModel = [NSRTCGroupChatViewModel modelWithViewCtrl:self];
    self.viewModel.toUsers = [self.users copy];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self setGroupChatRoomTitle];
}
- (void)setGroupChatRoomTitle{
    NSString *titleStr = [NSRTCChatManager shareManager].user.currentUserID;
    for (int i  = 0 ; i<self.users.count; i++) {
        NSString *lastTitleValue = [titleStr copy];
        NSString *currUser = self.users[i];
        titleStr = titleStr.length==0?currUser:[NSString stringWithFormat:@"%@,%@",titleStr,currUser];
        if (titleStr.length>15) {
            titleStr = [lastTitleValue copy];
            titleStr = [NSString stringWithFormat:@"%@...(%zd)",titleStr,(self.users.count + 1)];
            break;
        }
    }
    //
...
- (void)takePhoto {
    ...
    takeVC.modalPresentationStyle = UIModalPresentationFullScreen;
...
/**
 进入音频聊天
 */
- (void)goToAudioChatRoom {
//    NSRTCAVLivePhoneCallViewController *phoneCallPage = [NSRTCAVLivePhoneCallViewController takePhoneCall:NSRTCAVLivePhoneCallAudioCall toUser:self.toUser];
//    phoneCallPage.modalPresentationStyle = UIModalPresentationFullScreen;
//    [self presentViewController:phoneCallPage animated:YES completion:nil];
