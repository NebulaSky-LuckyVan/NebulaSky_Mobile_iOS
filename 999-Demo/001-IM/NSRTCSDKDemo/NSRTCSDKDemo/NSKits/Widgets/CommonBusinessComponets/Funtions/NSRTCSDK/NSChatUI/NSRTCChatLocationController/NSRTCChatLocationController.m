...
static NSString *const NSRTCLocationCellOnlyName = @"NSRTCLocationCellOnlyName";
static NSString *const NSRTCLocationCellDefault = @"NSRTCLocationCellDefault";
@interface NSRTCChatLocationController ()<MAMapViewDelegate, AMapSearchDelegate, UITableViewDelegate, UITableViewDataSource>
...
@implementation NSRTCChatLocationController
...
- (void)creatUI {
    ...
    [_tableView setSeparatorColor:[UIColor colorWithHex:0xefefef]];
...
- (void)hideCellSelectedImage:(BOOL)hidden index:(NSInteger)index {
    if (index <= self.aroundLocationArr.count) {
        NSRTCLocationCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
        if (cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSRTCLocationCell *cell;
    if (indexPath.row == 0) {
        cell = [tableView dequeueReusableCellWithIdentifier:NSRTCLocationCellOnlyName];
        if (!cell) {
            cell = [[NSRTCLocationCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSRTCLocationCellOnlyName cellStyle:NSRTCOnlyName];
        }
        cell.nameLabel.text = self.currentDetailLocationName;
        
    }
    else {
        cell = [tableView dequeueReusableCellWithIdentifier:NSRTCLocationCellDefault];
        if (!cell) {
            cell = [[NSRTCLocationCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSRTCLocationCellDefault cellStyle:NSRTCNameAndAddress];
        }
...
@implementation NSRTCLocationCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier cellStyle:(NSRTCLocationCellStyle)cellStyle{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        if (cellStyle == NSRTCOnlyName) {
            ...
            _addressLabel.textColor = [UIColor colorWithHex:0x989898];
